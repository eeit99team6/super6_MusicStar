...
public class MusiclistController {
	...
	public String method(String prodaction, MusicListBean bean, BindingResult bindingResult, Model model,
		...
		// errors.put("xxx1", context.getMessage("product.id.required", new String[]
		// {prodaction}, locale));
		// }
		// }
		//
	...
	// 刪除我的歌單 謙0314
	@RequestMapping(path = { "/deletemymusiclist" }, method = { RequestMethod.GET,
			RequestMethod.POST }, produces = "application/json;charset=UTF-8")
	@ResponseBody
	...
	// 修改我的歌單 謙
	@RequestMapping(path = { "/updatemymusiclist" }, method = { RequestMethod.GET,
			RequestMethod.POST }, produces = "application/json;charset=UTF-8")
	@ResponseBody
	public String updatemethod(MusicListBean bean, Model model) {
		Map<String, String> data = new HashMap<>();
		MusicListBean result = musicListService.update(bean);
		if (result == null) {
			data.put("action", "Update fail");
		} else {
			model.addAttribute("update", result);
		}
		return Parser.toJson(data);
